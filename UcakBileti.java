...
public class UcakBileti {
    public static void main(String[] args) {
        int KM, age, tip;
        double perKM = 0.1, tutar, bilet = 0;
        ...
        if ((KM > 0) && (age > 0)) {
            if (tip == 1) {
                if (age < 12) {
                    bilet = (tutar - cocukIndirimi);
                } else if (age < 24) {
                    bilet = (tutar - gencIndirimi);
                } else if (age < 65) {
                    bilet = tutar;
                } else {
                    bilet = (tutar - yasliIndirimi);
                }
            } else if (tip == 2) {
                if (age < 12) {
                    bilet = gidisDonusIndirim - cocukIndirimi;
                } else if (age < 24) {
                    bilet = gidisDonusIndirim - gencIndirimi;
                } else if (age < 65) {
                    bilet = gidisDonusIndirim;
                } else {
                    bilet = gidisDonusIndirim - yasliIndirimi;
                }
            } else {
                System.out.print("Hatalı Giriş Yaptınız. ");
            }
            System.out.print("Ödemeniz gereken ücret : " + bilet);
        } else {
